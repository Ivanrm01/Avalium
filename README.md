...
│   ├── post.njk
│   └── partials/                    (cabecera, pie, filtros, paginación)
│
├── blog.njk                         ◄── índice del blog (paginado de 9 en 9)
├── categoria.njk                    ◄── páginas /blog/categoria/...
├── sitemap.njk                      ◄── sitemap.xml
├── feed.njk                         ◄── feed.xml (RSS)
├── GUIA-SEO-BLOG.md                 ◄── cómo redactar cada artículo
│
├── package.json                     ◄── NUEVO
...
**Campos SEO**: además de los anteriores, el panel tiene un bloque de campos de SEO (título para Google, meta descripción, palabra clave, imagen para redes, preguntas frecuentes, autor y fecha de actualización). Todos son opcionales y tienen valor por defecto, pero rellenarlos es lo que separa un artículo publicado de un artículo posicionado. Cada campo lleva su propia explicación bajo el nombre, y el detalle está en **`GUIA-SEO-BLOG.md`**.
...
## ¿Y la página `/blog` (índice)?

✅ **Se actualiza automáticamente.** Cuando publicas un artículo nuevo desde el panel, aparece de inmediato en `/blog` (en cuanto Vercel termina el deploy, ~1 min).

**Cómo funciona el índice:**

- Los artículos se ordenan por fecha de publicación (los más recientes primero).
- **Se muestran 9 artículos por página.** A partir del décimo se crea sola una segunda página en `/blog/pagina-2`, luego `/blog/pagina-3`, y así sucesivamente, con la navegación numerada al final del listado. El artículo destacado va aparte, arriba, y solo en la primera página.
- Para cambiar ese número, edita la constante `ARTICULOS_POR_PAGINA` al principio de `.eleventy.js`. Se aplica también a las categorías.
- Los filtros de categoría **son páginas reales**, no botones: `/blog/categoria/aplazamientos`, `/blog/categoria/hipoteca-unilateral`, etc. Se generan solas a partir de las categorías que uses y también se paginan de 9 en 9. Una categoría con menos de 3 artículos se puede visitar pero no se indexa en Google (para no crear páginas de contenido pobre); en cuanto llega a 3, se indexa sola.
